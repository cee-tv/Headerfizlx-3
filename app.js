...
const _CAT_CHEVRON_SVG = `<svg class="cat-trigger-chevron" xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="transition:transform .2s;flex-shrink:0"><polyline points="6 9 12 15 18 9"/></svg>`;
const _CAT_LIST_SVG = `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>`;

function _buildCatDropdown(containerId, currentCat, onSelect) {
  ...
  const label = currentCat === 'All' ? 'All Categories' : currentCat;

  const trigger = document.createElement('button');
  trigger.className = 'quick-btn cat-dropdown-trigger';
  trigger.id = containerId + '-trigger';
  trigger.innerHTML = `${_CAT_LIST_SVG}<span class="cat-trigger-label">${label}</span>${_CAT_CHEVRON_SVG}`;

  const panel = document.createElement('div');
  panel.className = 'categories-dropdown-panel hidden pos-cat-panel';
  panel.id = containerId + '-panel';

  const allBtn = document.createElement('button');
  allBtn.className = 'category-btn' + (currentCat === 'All' ? ' active' : '');
  allBtn.innerText = 'All';
  allBtn.onclick = () => onSelect('All');
  panel.appendChild(allBtn);

  categories.forEach(c => {
    const btn = document.createElement('button');
    btn.className = 'category-btn' + (currentCat === c.name ? ' active' : '');
    btn.innerText = c.name;
    if (c.color) { btn.style.background = c.color; btn.style.borderColor = c.color; }
    btn.onclick = () => onSelect(c.name);
  ...
  trigger.onclick = (e) => {
    ...
    trigger.querySelector('.cat-trigger-chevron').style.transform = open ? '' : 'rotate(180deg)';
  };
...
function renderCategoriesUI() {
  _buildCatDropdown('categories', currentCategory, setCategory);
}

function renderLendingCategoriesUI() {
  _buildCatDropdown('lending-categories', currentLendingCategory, setLendingCategory);
}

// Close POS category dropdowns on outside click (registered once)
document.addEventListener('click', (e) => {
  ['categories', 'lending-categories'].forEach(id => {
    const wrapper = document.getElementById(id);
    const panel = document.getElementById(id + '-panel');
    const chevron = document.querySelector('#' + id + '-trigger .cat-trigger-chevron');
...
function renderCategoriesManagement() {
  ...
  if (categories.length === 0) {
    div.innerHTML = '<span style="color:var(--muted);font-size:13px;padding:6px 4px">No categories yet</span>';
    return;
  }
...
// Dropdown toggle for categories management
(function () {
  function setupCatDropdown() {
    const btn = document.getElementById('categories-dropdown-btn');
    const panel = document.getElementById('categories-management-list');
    const chevron = document.getElementById('categories-dropdown-chevron');
    if (!btn || !panel) return;
    ...
    document.addEventListener('click', (e) => {
      if (!document.getElementById('categories-dropdown-wrapper')?.contains(e.target)) {
        panel.classList.add('hidden');
        if (chevron) chevron.style.transform = '';
      }
    });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupCatDropdown);
  } else {
    setupCatDropdown();
  }
})();
...
async function initShift() {
  ...
  try {
    // Every role loads only their own open shift by name.
    // Admin's cashierName in Firestore is stored as 'CEO'.
    let q;
    const shiftName = currentUserRole === 'admin' ? 'CEO' : currentEmployeeName;
    if (shiftName) {
      q = query(collection(db, 'shifts'),
        where('status', '==', 'open'),
        where('cashierName', '==', shiftName));
    } else {
      // No role/name yet (app startup before login) — load any open shift for display only
      q = query(collection(db, 'shifts'), where('status', '==', 'open'));
    }
...
async function startNewShift() {
  ...
  try {
    ...
    await runTransaction(db, async (transaction) => {
      ...
      // Check globally — only one open shift allowed at a time across all accounts
      const globalQ = query(collection(db, 'shifts'),
        where('status', '==', 'open'),
        limit(1));
      ...
      if (!globalSnap.empty) {
        const openShift = globalSnap.docs[0].data();
        const openName = openShift.cashierName || openShift.openedBy || 'another account';
        throw new Error('GLOBAL_SHIFT_OPEN:' + openName);
      }
    ...
    if (err.message && err.message.startsWith('GLOBAL_SHIFT_OPEN:')) {
      const openName = err.message.split(':')[1];
      alert(`"${openName}" has an open shift. That shift must be ended before starting a new one.`);
    } else if (err.message === 'ALREADY_ACTIVE') {
    ...
    querySnapshot.forEach((doc) => {
      ...
      loaded.push({
        ...
        barcodes: Array.isArray(data.barcodes)
          ? data.barcodes.map(b => String(b).trim()).filter(Boolean)
        ...
        customPrices: Array.isArray(data.customPrices) ? data.customPrices.map(cp => ({ qty: Number(cp.qty), price: Number(cp.price) })).filter(cp => cp.qty > 0 && cp.price > 0) : []
      });
    });
    products = loaded;
    renderProducts();
...
let _newProductBarcodes = [];

function renderNewProductBarcodes() {
  const list = document.getElementById('new-product-barcodes-list');
  if (!list) return;
  list.innerHTML = '';
  _newProductBarcodes.forEach((bc, i) => {
    const tag = document.createElement('span');
    tag.className = 'barcode-tag';
    tag.innerHTML = `${bc} <button type="button" class="barcode-tag-remove" data-i="${i}" title="Remove">×</button>`;
    tag.querySelector('.barcode-tag-remove').onclick = () => {
      _newProductBarcodes.splice(i, 1);
      renderNewProductBarcodes();
    };
    list.appendChild(tag);
  });
}

function addNewProductBarcode(code) {
  if (!code) return;
  if (_newProductBarcodes.includes(code)) return;
  _newProductBarcodes.push(code);
  renderNewProductBarcodes();
}

function setNewProductBarcodes(arr) {
  _newProductBarcodes = Array.isArray(arr) ? [...arr] : [];
  renderNewProductBarcodes();
}
// ─────────────────────────────────────────────────────────────────────
...
function openEditProductModal(product) {
  ...
  setNewProductBarcodes(product.barcodes || []);
  ...
  resetCustomPriceRows(product.customPrices || []);
...
function setCategory(cat) {
  ...
  const label = document.querySelector('#categories-trigger .cat-trigger-label');
  const chevron = document.querySelector('#categories-trigger .cat-trigger-chevron');
  if (panel) {
    panel.querySelectorAll('.category-btn').forEach(b => b.classList.toggle('active', b.innerText.trim() === cat));
    panel.classList.add('hidden');
  }
...
function setLendingCategory(cat) {
  ...
  const panel = document.getElementById('lending-categories-panel');
  const label = document.querySelector('#lending-categories-trigger .cat-trigger-label');
  const chevron = document.querySelector('#lending-categories-trigger .cat-trigger-chevron');
  if (panel) {
    panel.querySelectorAll('.category-btn').forEach(b => b.classList.toggle('active', b.innerText.trim() === cat));
    panel.classList.add('hidden');
  }
...
function renderLendingCart() {
  ...
  // Calculate and display total profit for lending cart (use actual line totals)
  let lendingProfit = 0;
  lendingCart.forEach(item => {
    const cap = Number(item.capital || 0);
    if (item.unit && item.unit.toLowerCase() === 'kg') {
      const lineTotal = Number(item.total || 0);
      lendingProfit += lineTotal - cap * Number(item.weight || 0);
    } else {
      const lineTotal = Number(item.total ?? item.price * item.qty);
      lendingProfit += lineTotal - cap * Number(item.qty || 0);
    }
  });
  const lendingProfitEl = document.getElementById('lending-profit');
  if (lendingProfitEl) lendingProfitEl.innerText = formatCurrency(lendingProfit);
...
if (borrowerConfirm) borrowerConfirm.onclick = async () => {
  if (borrowerConfirm.disabled) return;
  const borrowerName = document.getElementById('borrower-name').value.trim();
  if (!borrowerName) {
    alert('Please enter borrower name');
    return;
  }
  borrowerConfirm.disabled = true;
  borrowerConfirm.textContent = 'Saving…';
  try {
    await saveLending(borrowerName);
  } finally {
    borrowerConfirm.disabled = false;
    borrowerConfirm.textContent = 'Confirm Lend';
  }
};

async function saveLending(borrowerName) {
  // Block if no active own shift
  const _lendShiftName = currentUserRole === 'admin' ? 'CEO' : currentEmployeeName;
  if (!currentShift || !currentShift.id || currentShift.status !== 'open' ||
      String(currentShift.cashierName || '').trim() !== String(_lendShiftName || '').trim()) {
    alert('You must start your own shift before recording a lending.');
    return;
  }
...
function renderProducts() {
  ...
  if (q) {
    filtered = filtered.filter(p => cleanStr(p.name).includes(q) || p.barcodes.some(b => cleanStr(b).includes(q)));
  }
  ...
  filtered.forEach((p) => {
    ...
    const dealLine = validDeals.length > 0
      ? `<span class="btn-deal">${validDeals.map(d => `${d.qty} for ₱${d.price}`).join(' · ')}</span>`
      : '';
    const priceLabel = isKg ? `${formatCurrency(p.price)}/kg` : formatCurrency(p.price);
    btn.innerHTML = `
      <span class="btn-name">${p.name}</span>
      ${dealLine}
      <div class="btn-bottom-row">
...
/**
 * Calculate the line total applying custom bulk-price deals.
 * customPrices: [{qty: 3, price: 5}] means "3 pieces for ₱5 total"
 * Rules:
 *   - A deal is only applied if it's cheaper than buying the same qty at regular price.
 *   - Among all valid deals, we pick the best combination (lowest total) via dynamic programming.
 *   - If no deal saves money, fall back to regular price × qty.
 * Returns { total, dealLabel }
 */
function calcLineTotal(price, qty, customPrices) {
  const regularTotal = Number((price * qty).toFixed(2));
  // Filter to only deals that are genuinely cheaper than their equivalent regular price
  const deals = (customPrices || []).filter(d => d.qty > 0 && d.price > 0 && d.price < d.qty * price);
  if (deals.length === 0 || qty === 0) {
    return { total: regularTotal, dealLabel: '' };
  }

  // DP: dp[i] = { cost, usedDeals } for buying exactly i pieces at minimum cost
  const dp = new Array(qty + 1).fill(null).map(() => ({ cost: Infinity, deals: [] }));
  dp[0] = { cost: 0, deals: [] };
  for (let i = 1; i <= qty; i++) {
    // Option 1: buy this piece at regular unit price
    if (dp[i - 1].cost + price < dp[i].cost) {
      dp[i] = { cost: dp[i - 1].cost + price, deals: dp[i - 1].deals };
    }
    // Option 2: apply a deal ending at i
    for (const deal of deals) {
      if (i >= deal.qty) {
        const candidate = dp[i - deal.qty].cost + deal.price;
        if (candidate < dp[i].cost) {
          dp[i] = { cost: candidate, deals: [...dp[i - deal.qty].deals, deal] };
        }
      }
    }
  }

  const bestTotal = Number(dp[qty].cost.toFixed(2));
  // Only use deals if they beat the regular total
  if (bestTotal >= regularTotal) {
    return { total: regularTotal, dealLabel: '' };
  }

  // Build a readable label of the deals actually used
  const usedDeals = dp[qty].deals;
  const dealCounts = new Map();
  for (const d of usedDeals) {
    const key = `${d.qty} for ₱${d.price}`;
    dealCounts.set(key, (dealCounts.get(key) || 0) + 1);
  }
  const dealLabel = [...dealCounts.entries()]
    .map(([label, count]) => count > 1 ? `${count}× (${label})` : label)
    .join(', ');

  return { total: bestTotal, dealLabel };
}
...
function addToCart(product) {
  ...
  if (existing) {
    existing.qty += 1;
    const { total, dealLabel } = calcLineTotal(existing.price, existing.qty, existing.customPrices);
    existing.total = total;
    existing.dealLabel = dealLabel;
  } else {
    const cp = Array.isArray(product.customPrices) ? product.customPrices : [];
    const { total, dealLabel } = calcLineTotal(Number(product.price), 1, cp);
    cart.push({
      name: product.name,
      price: Number(product.price),
      capital: Number(product.capital || 0),
...
function renderCart() {
  ...
  cart.forEach((item, idx) => {
    ...
    if (item.unit && item.unit.toLowerCase() === 'kg') {
      ...
      lineTotalNumber = Number(item.total ?? item.price * item.qty);
      lineText = `${item.name} (${item.unit}) x${Number(item.qty).toFixed(2)} = ${formatCurrency(lineTotalNumber)}`;
    }
    ...
    // Show deal badge if a custom price deal is active
    if (item.dealLabel) {
      const badge = document.createElement('span');
      badge.className = 'cart-deal-badge';
      badge.textContent = '🏷 ' + item.dealLabel;
  ...
  // Calculate and display total profit for the cart (deal-aware: use actual line total minus capital cost)
  let totalProfit = 0;
  cart.forEach(item => {
    const cap = Number(item.capital || 0);
    if (item.unit && item.unit.toLowerCase() === 'kg') {
      const lineTotal = Number(item.total || 0);
      totalProfit += lineTotal - cap * Number(item.weight || 0);
    } else {
      const lineTotal = Number(item.total ?? item.price * item.qty);
      totalProfit += lineTotal - cap * Number(item.qty || 0);
    }
  });
...
function openReceiptModal(saleObj = null) {
  ...
  itemsSource.forEach(item => {
    ...
    // Resolve capital — stored on item (new sales) or looked up from products (history)
    const cap = Number(item.capital ?? (products.find(p => p.name === item.name)?.capital) ?? 0);

    if (item.unit && item.unit.toLowerCase() === 'kg') {
      const weight = item.weight || 0;
      lineTotal = Number(item.lineTotal ?? item.total ?? 0);
      const lineProfit = (item.price - cap) * weight;
      ...
      li.appendChild(mkSpan('rcol-profit', '+' + formatCurrency(lineProfit)));
    } else {
      const qty = item.qty || 0;
      lineTotal = Number(item.lineTotal ?? item.total ?? (item.price * qty));
      const lineProfit = lineTotal - cap * qty;
      totalProfit += lineProfit;
      li.appendChild(mkSpan('rcol-name', item.name));
  ...
  const receiptProfitEl = document.getElementById('receipt-profit');
  if (receiptProfitEl) receiptProfitEl.innerText = formatCurrency(totalProfit);
...
if (receiptSave) receiptSave.onclick = async () => {
  ...
  const itemsForSave = cart.map(i => {
    ...
    if (i.unit && i.unit.toLowerCase() === 'kg') {
      ...
      it.qty = i.qty;
      it.lineTotal = Number(Number(i.total ?? (i.price * i.qty)).toFixed(2));
      if (i.dealLabel) it.dealLabel = i.dealLabel;
    }
  ...
  const expectedShiftName = currentUserRole === 'admin' ? 'CEO' : currentEmployeeName;
  if (!currentShift || !currentShift.id || currentShift.status !== 'open' ||
      String(currentShift.cashierName || '').trim() !== String(expectedShiftName || '').trim()) {
    console.warn('Receipt save blocked - no active shift for current user', { currentShift, expectedShiftName });
    alert('You must start your own shift before checkout.');
    resetSaveBtn();
    return;
  }
...
async function loadProfits() {
  ...
  try {
    ...
    qSnap.forEach(docSnap => {
      ...
      (s.items || []).forEach(it => {
        // Use actual saved capital + lineTotal (deal-aware) so bulk deals are calculated correctly
        let cap = it.capital != null ? Number(it.capital) : (() => {
          const p = products.find(pr => pr.name === it.name && pr.unit === it.unit);
          return p != null ? Number(p.capital) : null;
        })();
        // Skip line if capital is unresolvable to avoid inflating profit
        if (cap == null || !isFinite(cap)) return;
        cap = Math.max(0, cap);
        if (it.unit && it.unit.toLowerCase() === 'kg') {
          const qty = Number(it.weight || 0);
          const lineTotal = Number(it.lineTotal ?? it.total ?? (Number(it.price || 0) * qty));
          if (!isFinite(lineTotal) || !isFinite(qty)) return;
          saleProfit += lineTotal - cap * qty;
        } else {
...
let financeRange = { start: null, end: null, preset: 'daily' };
...
async function loadFinancePage() {
  if (financeRange.preset === 'daily' && !financeRange.start) {
    updateFinanceRange('daily');
    return;
  }
  try {
...
function updateCheckoutButtonState() {
  ...
  if (!canCheckout()) {
    ...
    // Only log when a user is actually logged in — pre-login state is expected.
    if (currentUserRole) {
  ...
  allNames.forEach(cat => {
    ...
    byCat[cat].forEach(p => {
      ...
      if (!productsEditMode) {
        ...
        const profitLabel = (profitPerUnit >= 0 ? '+' : '') + formatCurrency(profitPerUnit);
        titleDiv.innerHTML += `
          <div class="pec-info-row">
            ...
            <span class="pec-capital-tag ${profitClass}">Profit: ${profitLabel}</span>
            ${expiryBadge}
          </div>
        `;
      } else {
      ...
      if (productsEditMode) {
        ...
        inputsDiv.innerHTML = `
          ...
          <div style="grid-column: span 2;"><label>Barcode</label>
            <div style="display:flex;gap:6px">
              <input type="text" class="edit-barcode" placeholder="Barcodes (comma-separated)" value="${(p.barcodes || []).join(', ')}" style="flex:1" />
        ...
        async function autoSave() {
          ...
          const newBarcodes = barcodeIn
            ? barcodeIn.value.split(',').map(b => b.trim()).filter(Boolean)
            : (p.barcodes || []);
          ...
          try {
            await updateDoc(doc(db, 'products', id), {
              ...
              capital: newCapital, profit: newProfit, category: newCategory, barcodes: newBarcodes,
              expirationDates: newExpirationDates, customPrices: existingCustomPrices
            });
            ...
            if (idx !== -1) {
              products[idx] = { ...products[idx], name: newName, price: newPrice,
                stock: newStock, capital: newCapital, profit: newProfit, category: newCategory, barcodes: newBarcodes,
                expirationDates: newExpirationDates, customPrices: existingCustomPrices };
            }
            playSfx('success');
        ...
        if (editScanBarcodeBtn) editScanBarcodeBtn.onclick = () => {
          openBarcodeScanner('product-form', (code) => {
            const barcodeIn = row.querySelector('.edit-barcode');
            if (barcodeIn) {
              const existing = barcodeIn.value.split(',').map(b => b.trim()).filter(Boolean);
              if (!existing.includes(code)) existing.push(code);
              barcodeIn.value = existing.join(', ');
              scheduleAutoSave();
            }
          });
        };
    ...
    if (modalEditIndex !== null) {
      ...
      if (existing) {
        ...
        lendingCart.push({
          ...
          capital: Number(modalProduct.capital || 0),
          unit: 'Kg',
...
function addCustomPriceRow(qty, price) {
  const list = document.getElementById('new-product-custom-prices-list');
  if (!list) return;
  const row = document.createElement('div');
  row.className = 'custom-price-row';
  row.innerHTML = `
    <span class="cp-label">Buy</span>
    <input type="number" class="cp-qty-input" min="1" step="1" placeholder="Qty" value="${qty != null && qty !== '' ? qty : ''}" style="max-width:70px" />
    <span class="cp-label">pcs for ₱</span>
    <input type="number" class="cp-price-input" min="0.01" step="0.01" placeholder="Price" value="${price != null && price !== '' ? price : ''}" style="max-width:90px" />
    <button type="button" class="quick-btn custom-price-row-remove" title="Remove"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
  `;
  row.querySelector('.custom-price-row-remove').onclick = () => row.remove();
  list.appendChild(row);
}

function resetCustomPriceRows(entries) {
  const list = document.getElementById('new-product-custom-prices-list');
  if (!list) return;
  list.innerHTML = '';
  (entries || []).forEach(e => addCustomPriceRow(e.qty, e.price));
}

function collectCustomPriceRows() {
  const list = document.getElementById('new-product-custom-prices-list');
  if (!list) return [];
  const entries = [];
  list.querySelectorAll('.custom-price-row').forEach(row => {
    const qty = Number(row.querySelector('.cp-qty-input').value);
    const price = Number(row.querySelector('.cp-price-input').value);
...
const newProductAddCustomPriceBtn = document.getElementById('new-product-add-custom-price');
if (newProductAddCustomPriceBtn) newProductAddCustomPriceBtn.onclick = () => addCustomPriceRow('', '');

if (addProductBtn) addProductBtn.onclick = () => {
  document.getElementById('new-product-name').value = '';
  setNewProductBarcodes([]);
  const barcodeTyped = document.getElementById('new-product-barcode-input');
  if (barcodeTyped) barcodeTyped.value = '';
...
if (addProductSave) addProductSave.onclick = async () => {
  if (_productSaving) return;
  const name = document.getElementById('new-product-name').value.trim();
  const barcodes = [..._newProductBarcodes];
  ...
  const customPrices = collectCustomPriceRows();
  ...
  try {
    if (editingProductId) {
      await safeWrite('update', 'products', { name, barcodes, unit, category, price, capital, profit, stock, expirationDates, customPrices }, editingProductId);
    } else {
      await safeWrite('add', 'products', { name, barcodes, unit, category, price, capital, profit, stock, expirationDates, customPrices });
    }
    editingProductId = null;
    document.querySelector('#add-product-modal .add-product-header h4').textContent = 'Add Product';
    addProductSave.textContent = 'Save Product';
...
async function handleBarcodeScanResult(decodedText) {
  ...
  const match = products.find(p => p.barcodes.some(b => b.trim() === code));
  if (match) {
    addToCart(match);
...
const newProductBarcodeAdd = document.getElementById('new-product-barcode-add');
const newProductBarcodeInput = document.getElementById('new-product-barcode-input');
if (newProductBarcodeAdd) newProductBarcodeAdd.onclick = () => {
  const code = newProductBarcodeInput ? newProductBarcodeInput.value.trim() : '';
  if (code) { addNewProductBarcode(code); if (newProductBarcodeInput) newProductBarcodeInput.value = ''; }
};
if (newProductBarcodeInput) newProductBarcodeInput.addEventListener('keydown', e => {
  if (e.key === 'Enter') { e.preventDefault(); if (newProductBarcodeAdd) newProductBarcodeAdd.click(); }
});
...
function updateNavAccess() {
  ...
  document.querySelectorAll('.nav-btn').forEach(b => {
    ...
    if (currentUserRole === 'cashier') {
      ...
      if (txt === 'Stocks' || txt === 'Products' || txt === 'Admin' || txt === 'Remits' || txt === 'Profits' || txt === 'Finance') b.style.display = 'none';
...
if (loginBtn) loginBtn.onclick = async () => {
  ...
  try {
    ...
    if (!qSnap.empty) {
      ...
      if (remember) saveSession(); else clearSession();
    } else {
...
// Default receipt page to Today (local date, not UTC)
const _d = new Date();
const _rcptToday = _d.getFullYear() + '-' + String(_d.getMonth()+1).padStart(2,'0') + '-' + String(_d.getDate()).padStart(2,'0');
if (startDateInput && !startDateInput.value) { startDateInput.value = _rcptToday; historyFilters.startDate = _rcptToday; }
if (endDateInput && !endDateInput.value) { endDateInput.value = _rcptToday; historyFilters.endDate = _rcptToday; }
loadSalesHistory();
  ...
  getDocs(q).then(qSnap => {
    ...
    qSnap.forEach(docSnap => {
      ...
      if (unpaid > 0) {
        ...
        // Compute profit for this lending entry
        // capital stored on item (new records) or fall back to live products (old records)
        let entryProfit = 0;
        (l.items || []).forEach(item => {
          const cap = item.capital != null
            ? Number(item.capital)
            : Number(products.find(p => p.name === item.name)?.capital || 0);
          const lineTotal = Number(item.total || 0);
          if (item.unit && item.unit.toLowerCase() === 'kg') {
            entryProfit += lineTotal - cap * Number(item.weight || 0);
          } else {
            entryProfit += lineTotal - cap * Number(item.qty || 0);
          }
        });
        totalProfit += entryProfit;

        const lendingDiv = document.createElement('div');
        lendingDiv.style.cssText = 'border-bottom:1px solid var(--border);padding-bottom:10px;margin-bottom:10px';
        lendingDiv.innerHTML = `<div style="font-weight:700;margin-bottom:4px">Lending on ${toDisplayDate(l.timestamp).toLocaleString()}</div>`;
        const itemsList = document.createElement('ul');
        itemsList.style.marginBottom = '4px';
        (l.items || []).forEach(item => {
          if (!item.paid) {
            const li = document.createElement('li');
            const cap = Number(item.capital || 0);
            ...
            let itemProfit = 0;
            if (item.unit && item.unit.toLowerCase() === 'kg') {
              itemProfit = lineTotal - cap * Number(item.weight || 0);
            } else {
              itemProfit = lineTotal - cap * Number(item.qty || 0);
            }
            const qtyStr = item.unit && item.unit.toLowerCase() === 'kg' ? `${Number(item.weight).toFixed(2)}kg` : `x${item.qty}`;
            const profitStr = cap > 0 ? ` <span style="color:#27ae60;font-size:11px">(+${formatCurrency(itemProfit)})</span>` : '';
            li.innerHTML = `${item.name} ${qtyStr} = ${formatCurrency(lineTotal)}${profitStr}`;
            itemsList.appendChild(li);
          }
        });
        lendingDiv.appendChild(itemsList);

        const unpaidP = document.createElement('p');
        unpaidP.style.marginBottom = '6px';
        const profitNote = entryProfit > 0 ? ` &nbsp;<span style="color:#27ae60;font-size:12px">Profit: ${formatCurrency(entryProfit)}</span>` : '';
        unpaidP.innerHTML = `Total Unpaid: <strong>${formatCurrency(unpaid)}</strong>${profitNote}`;
        lendingDiv.appendChild(unpaidP);

        // Edit / Delete buttons
        const entryActions = document.createElement('div');
        entryActions.style.cssText = 'display:flex;gap:8px';

        const editBtn = document.createElement('button');
        editBtn.className = 'checkout-btn';
        editBtn.style.cssText = 'font-size:12px;padding:4px 14px';
        editBtn.textContent = 'Edit';
        editBtn.onclick = () => openEditLendingModal(docSnap.id, l, borrowerName);

        const delBtn = document.createElement('button');
        delBtn.className = 'remove-btn';
        delBtn.style.cssText = 'font-size:12px;padding:4px 14px';
        delBtn.textContent = 'Delete';
        delBtn.onclick = () => deleteLendingEntry(docSnap.id, borrowerName);

        entryActions.appendChild(editBtn);
        entryActions.appendChild(delBtn);
        lendingDiv.appendChild(entryActions);

        itemsEl.appendChild(lendingDiv);
      }
    });

    if (itemsEl.innerHTML === '') itemsEl.innerText = 'No unpaid entries.';

    totalEl.innerText = formatCurrency(totalUnpaid);
    const profitEl = document.getElementById('lending-details-profit');
    if (profitEl) profitEl.innerText = formatCurrency(totalProfit);
    modal.classList.remove('hidden');
    modal.setAttribute('aria-hidden', 'false');
  }).catch(err => {
...
// ── Lending entry delete ────────────────────────────────────────────────────
async function deleteLendingEntry(docId, borrowerName) {
  if (!confirm(`Delete this lending entry for "${borrowerName}"? This cannot be undone.`)) return;
  try {
    await safeWrite('delete', 'lendings', null, docId);
    loadBorrowersList();
    closeLendingDetailsModal();
    // Re-open details in case there are other entries for this borrower
    setTimeout(() => showBorrowerDetails(borrowerName), 400);
  } catch (err) {
    console.error('Failed to delete lending entry', err);
    alert('Failed to delete entry. Check console.');
  }
}
window.deleteLendingEntry = deleteLendingEntry;

// ── Lending entry edit ──────────────────────────────────────────────────────
let _editLendingDocId = null;
let _editLendingOriginalBorrower = null;

function openEditLendingModal(docId, data, borrowerName) {
  _editLendingDocId = docId;
  _editLendingOriginalBorrower = borrowerName;

  document.getElementById('edit-lending-name').value = data.borrowerName || borrowerName;

  const container = document.getElementById('edit-lending-items');
  container.innerHTML = '';

  (data.items || []).forEach((item, idx) => {
    const isKg = item.unit && item.unit.toLowerCase() === 'kg';

    const row = document.createElement('div');
    row.style.cssText = 'display:flex;align-items:center;gap:8px;flex-wrap:wrap';

    const nameSpan = document.createElement('span');
    nameSpan.style.cssText = 'flex:1;min-width:100px;font-weight:600';
    nameSpan.textContent = item.name;

    const qtyInput = document.createElement('input');
    qtyInput.type = 'number';
    qtyInput.step = isKg ? '0.01' : '1';
    qtyInput.min = '0';
    qtyInput.value = isKg ? Number(item.weight).toFixed(2) : item.qty;
    qtyInput.style.cssText = 'width:80px;text-align:center';
    qtyInput.title = isKg ? 'Weight (kg)' : 'Quantity';
    qtyInput.dataset.price = item.price;
    qtyInput.dataset.iskg = isKg ? '1' : '0';

    const unitSpan = document.createElement('span');
    unitSpan.style.color = 'var(--muted)';
    unitSpan.textContent = isKg ? 'kg' : 'pcs';
    ...
    rowTotalSpan.style.cssText = 'min-width:80px;text-align:right;font-weight:600';
    rowTotalSpan.dataset.rowtotal = idx;
    rowTotalSpan.textContent = formatCurrency(item.total);

    qtyInput.oninput = () => {
      const qty = parseFloat(qtyInput.value) || 0;
      const price = parseFloat(qtyInput.dataset.price) || 0;
      rowTotalSpan.textContent = formatCurrency(qty * price);
      recalcEditLendingTotal();
    };
    ...
    row.appendChild(rowTotalSpan);
    container.appendChild(row);
  });

  recalcEditLendingTotal();
  document.getElementById('edit-lending-modal').classList.remove('hidden');
  document.getElementById('edit-lending-modal').setAttribute('aria-hidden', 'false');
}
window.openEditLendingModal = openEditLendingModal;

function recalcEditLendingTotal() {
  let total = 0;
  document.querySelectorAll('#edit-lending-items input[type="number"]').forEach(input => {
    total += (parseFloat(input.value) || 0) * (parseFloat(input.dataset.price) || 0);
  });
  document.getElementById('edit-lending-total').textContent = formatCurrency(total);
}

function closeEditLendingModal() {
  document.getElementById('edit-lending-modal').classList.add('hidden');
  document.getElementById('edit-lending-modal').setAttribute('aria-hidden', 'true');
  _editLendingDocId = null;
}
window.closeEditLendingModal = closeEditLendingModal;

async function saveEditLending() {
  if (!_editLendingDocId) return;
  const newBorrowerName = document.getElementById('edit-lending-name').value.trim();
  if (!newBorrowerName) return alert('Borrower name cannot be empty.');

  try {
    const docRef = doc(db, 'lendings', _editLendingDocId);
    const docSnap = await getDoc(docRef);
    if (!docSnap.exists()) return alert('Lending entry not found.');

    const originalItems = docSnap.data().items || [];
    const inputs = document.querySelectorAll('#edit-lending-items input[type="number"]');
    ...
    const newItems = originalItems.map((item, idx) => {
      ...
      const isKg = input.dataset.iskg === '1';
      return { ...item, [isKg ? 'weight' : 'qty']: qty, total: qty * price };
    });

    const newTotal = newItems.reduce((s, i) => s + (i.total || 0), 0);

    await safeWrite('update', 'lendings', { borrowerName: newBorrowerName, items: newItems, total: newTotal }, _editLendingDocId);

    closeEditLendingModal();
    closeLendingDetailsModal();
    loadBorrowersList();
    setTimeout(() => showBorrowerDetails(newBorrowerName), 400);
  } catch (err) {
    console.error('Failed to save lending edit', err);
    alert('Failed to save changes. Check console.');
  }
}
window.saveEditLending = saveEditLending;
...
function renderEloadSummary(txs) {
  ...
  txs.forEach(t => {
    ...
    // Accumulate daily profit (fee = commission = profit); key by YYYY-MM-DD for stable sorting
    const dayKey = t.date && /^\d{4}-\d{2}-\d{2}$/.test(t.date) ? t.date : null;
    if (dayKey) {
      if (!dailyProfit[dayKey]) dailyProfit[dayKey] = 0;
      dailyProfit[dayKey] += Number(t.fee || 0);
    }
  });
  ...
  const totalCommEl = document.getElementById('eload-total-commission');
  if (totalCommEl) totalCommEl.textContent = formatCurrency(totalFee);

  // Render daily profit breakdown
  const profitListEl = document.getElementById('eload-profits-list');
  const profitTotalEl = document.getElementById('eload-total-profit');
  if (profitListEl) {
  ...
  if (profitTotalEl) profitTotalEl.textContent = formatCurrency(totalFee);
}
...
(function () {
  ...
  // Hide banner if app is already running as standalone (installed)
    ...
    if (copyBtn) {
      copyBtn.onclick = async () => {
        try {
          ...
          copyBtn.classList.add('note-copy-btn--copied');
          ...
          setTimeout(() => {
            copyBtn.classList.remove('note-copy-btn--copied');
